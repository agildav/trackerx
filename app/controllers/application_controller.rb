...
class ApplicationController < ActionController::API
  ...
  rescue_from(ActionController::ParameterMissing) { |err| handle_action_controller_parameter_missing_error(err) }
  rescue_from(ActionController::RoutingError) { |err| handle_action_controller_routing_error(err) }
  rescue_from(ActiveModel::ForbiddenAttributesError) { |err| handle_active_model_forbidden_attributes_error(err) }
  rescue_from(RestError) { |err| handle_rest_error(err) }
  ...
  def handle_forbidden_error(err)
    logger.error(err&.message)

    render_error_response(err&.message, :forbidden, 'forbidden_error')
  end

  def handle_invalid_credentials_error(err)
    logger.error(err&.message)

    render_error_response(err&.message, :bad_request, 'request_error')
  end

  def handle_rest_error(err)
    logger.error([err&.message, err&.errors])

    render_error_response(err&.message, :bad_request, 'request_error', err&.errors)
  end
end
